        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full bg-white/5">
                    <thead>
                        <tr>
                            <th className="py-2 px-4 border-b border-neutral-700 text-left text-sm font-semibold text-gray-300">Title</th>
                            <th className="py-2 px-4 border-b border-neutral-700 text-left text-sm font-semibold text-gray-300">Slug</th>
                            <th className="py-2 px-4 border-b border-neutral-700 text-left text-sm font-semibold text-gray-300">Published At</th>
                    ...
                    <tbody>
                        {artPosts.map((post) => (
                            <tr key={post._id}>
                                <td className="py-2 px-4 border-b border-neutral-800 text-sm text-gray-400">{post.title}</td>
                                <td className="py-2 px-4 border-b border-neutral-800 text-sm text-gray-400">{post.slug}</td>
                                <td className="py-2 px-4 border-b border-neutral-800 text-sm text-gray-400">{new Date(post.publishedAt).toLocaleDateString()}</td>
                                <td className="py-2 px-4 border-b border-neutral-800 text-sm">
                                    <Link href={`/x/artposts/edit/${post.slug}`}>
