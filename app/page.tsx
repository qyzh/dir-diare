        ...
        <main className="flex flex-col">
          ...
          <div className="">
            <span className="webcontent">
              ...
              interface design &running . My name import Syauqi Ashadullah and am
              currently living in Bandung, West Java.
            </span>
          </div>
          <div>
          ...
          <div className="ml-4 flex flex-col space-y-2 my-2">
            ...
            {postlist.map((post) => (
              <div key={post._id} className="ml-6">
                <span className="webtree">└──</span>
                <Link className="group" href={`/w/${post.slug}`}>
                  <span className="webcontent">{post.title}</span>
                </Link>
            ...
            {artlist.map((lab) => (
              <div key={lab.slug} className="ml-6">
                <Link className="group" href={`/l/${lab.slug}`}>
                  <span className="webtree">└──</span>
                  <span className="webcontent">{lab.metadata.title}</span>
                </Link>
                <span className="webformat">.mdx</span>
              </div>
            ))}
